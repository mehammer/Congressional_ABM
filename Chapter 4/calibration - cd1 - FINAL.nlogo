breed [resident residents] ;I always end up coding the singular and plural backwards.  It is consistent throughout
...
;most of these attributes aren't used now but are there for future reference.
  ...
  c1_distance ;stores distance between voter and candidate 0
  c2_distance ;stores distance between voter and candidate 1
...
candidate-own [
  ...
  nominate_dim1 ;float score on a scale from -1.0 to 1.0
  nominate_dim2 ;float score on a scale from -1.0 to 1.0
  adaptation ;attribute that stores the candidate's strategy
  polling ;tracks the number of voters supporting the candidate in the campaign time steps
  dim1_diff ;how much the candidate's NOMINATE dimension 1 score changes in a time step bc of strategy
  dim2_diff ;how much the candidate's NOMINATE dimension 1 score changes in a time step bc of strategy
]
...
;race1 and col (education) are also weights that modify the base dim 1 score.
    ...
    if voter_type = "MFCR" [set nominate_dim1 rep_dim1 + race1 + w + col]
...
to politicians
  ...
  ask candidates 0 [ ;Qasim Rashid
    ...
    set xcor -5
    set ycor 5
    set nominate_dim1 -0.252
    set nominate_dim2 -0.055
...
to campaign
  ...
  ;messaging value(s) created in the BehaviorSpace experiment

  ;voters assess their distance
  ;distance from candidate 0
  ...
  ifelse count resident with [poll = "C1"] > 0 ;only sets a value if there are any supporters for that candidate.  Otherwise puts value of "null"
    [set cohort01 mean [nominate_dim1] of resident with [poll = "C1"]] ;cohort01 = mean dimension 1 score all supporters of candidate 0
    [set cohort01 "null"]
  ifelse count resident with [poll = "C1"] > 0 ;only sets a value if there are any supporters for that candidate.  Otherwise puts value of "null"
    [set cohort02 mean [nominate_dim2] of resident with [poll = "C1"]] ;cohort02 = mean dimension 2 score all supporters of candidate 0
    [set cohort02 "null"]
  ifelse count resident with [poll = "C2"] > 0 ;only sets a value if there are any supporters for that candidate.  Otherwise puts value of "null"
  [set cohort11 mean [nominate_dim1] of resident with [poll = "C2"]] ;cohort11 = mean dimension 1 score all supporters of candidate 1
    [set cohort11 "null"]
  ifelse count resident with [poll = "C2"] > 0 ;only sets a value if there are any supporters for that candidate.  Otherwise puts value of "null"
  [set cohort12 mean [nominate_dim2] of resident with [poll = "C2"]] ;cohort12 = mean dimension 1 score all supporters of candidate 1
    [set cohort12 "null"]
 ...
 ask candidates 0 [
  ...
  let old-dim2 nominate_dim2
        ...
        if adaptation = "Hunter" ;adjust to the mean values of all voters
        ...
        if adaptation = "Pragmatist"
            [if cohort_cand1 > 0 ;checks that the
             [if cohort_cand1 < cohort_cand2 ;the candidate only implements the strategy if the other candidate has more voters
             ;ifelse function determines whether the messaging parameter adds or subtracts from the current NOMINATE score
             ;intended function: if the mean dim1 score held in cohort01 is less than the opposing candidate's NOMINATE dim 1 score, then ADD messaging
             ;otherwise, if the score is higher, SUBTRACT messaging
              [ifelse cohort01 < [nominate_dim1] of candidates 1
                [set nominate_dim1 nominate_dim1 + messaging] ;adds messaging
...
ask candidates 1 [ ;should all be the same as for candidate 0
...
to elect
 ...
 ;if the distance to EITHER candidate is less than alienation, they go to vote
   ask resident with [neighborhood = 1 ]
...
This chapter implements an innovative method for calibrating voter and candidate preferences in agent-based models of elections.  The congressional district-level models in this chapter incorporate multiple data sources to replicate the 2020 U.S. House elections in Virginia at full scale.  The chapter explains the multi-step process for developing a voter dataset of race, gender, and education data to assign agents to a voter type.  These empirically-calibrated voters are modeled along with candidates whose ideal points are calibrated with text data, generated in Chapter 3, and who choose from available campaign strategies first discussed in Chapter 2.  The model is validated by comparing the replicated election results of the 2020 U.S. House elections in Virginia to both the real aggregate vote totals for each U.S. House district and to the best available estimates of vote totals by voter type.  

The work of generating agents with intersectional identities allows for each agent to be randomly assigned a NOMINATE score within a range determined by the estimated voting preferences of that group.  The following process accomplishes this goal:
•	Step 1: Assign a mean value of the base voters for each party.  This mean value is taken from the median of each party’s NOMINATE score distribution in the 117th U.S. House (the House chosen in the 2020 general election).  This is not to imply that the U.S. House is ideologically representative of all voters in the U.S., nor even of each party’s own voter base.  It is simply the best reasonable choice mov-ing from the assumption that a majority of voters generated the NOMINATE dis-tribution in Congress and, therefore, that they are capable of voting in such a way as to generate such a distribution.  Whether or not this distribution is “representa-tive” in a meaningful way is a separate discussion.  The specific Dimension 1 mean values chosen are -0.393 for Democratic voters and 0.397 for Republican voters.
•	Step 2: Even within demographic groups, voters are heterogeneous.  To account for this uncertainty, the model assigns the base value to each voter group, by par-ty, as a random normal distribution with small tails.  The model sweeps through multiple possible values for the standard deviation of the mean.
•	Step 3: To account for the influence of race, education and gender on a greater likely preference for Democratic candidates, these three variables are parameter-ized as weights on the base mean value.  Multiple values for each weight are test-ed in a parameter sweep.  The weights modify a given voter’s NOMINATE score by the value of the weight.  The direction of the weight is always negative, as each of these weights increases the likelihood of the voter being more liberal and liberal scores are negative on the NOMINATE scale.



## HOW TO USE IT

The model is set for 4 campaign time steps, in which candidates attempt to attract voters by changing their own NOMINATE position. The 5th time step is the election, in which voters 1) decide to particpate in the election based on their Engagement threshold and 2) choose a candidate based on shortest Euclidean distance.  The vote is counted in Interface monitors and BehaviorSpace reporters.
...
@#$#@#$#@
<experiments>
  <experiment name="va01 - cand sweep simple centers" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="5"/>
  ...
  <experiment name="va01 - 5run" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="5"/>
    <metric>count resident with [voter = 1]</metric>
  ...
  <experiment name="va01 - 10run" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="5"/>
    <metric>count resident with [voter = 1]</metric>
  ...
  <experiment name="va01 - candidate distance" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="5"/>
    <metric>median [c1_distance] of resident with [voter = 1]</metric>
