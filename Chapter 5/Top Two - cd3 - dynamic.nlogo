...
;;THE n-of VALUE IS THE REAL ESTIMATED SUBPOPULATION NUMBER FOR THE DISTRICT FROM THE POP_DISTRIBUTION FILE
  ...
  ask n-of 3208 resident with [race = "none"][set race "minority" set voter_type "MFHR"]
...
<experiments>
  ...
  <experiment name="va03 -5runs" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10"/>
    <metric>count resident with [voter = 1]</metric>
  ...
  <experiment name="va03 -10runs" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10"/>
    <metric>count resident with [voter = 1]</metric>
